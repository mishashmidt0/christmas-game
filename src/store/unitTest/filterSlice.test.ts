...
test('should be change isActive from form toys', () => {
  const endState = filterValueSlice.reducer(
    initialState,
    changeForm({ name: FilterName.bell, isActive: true }),
  );
...
test('should be change isActive from color', () => {
  const endState = filterValueSlice.reducer(
    initialState,
    changeColor({ name: FilterName.red, isActive: true }),
  );
...
test('should be change isActive from size', () => {
  const endState = filterValueSlice.reducer(
    initialState,
    changeSize({ name: FilterName.small, isActive: true }),
  );
...
test('should be change isActive from favorite', () => {
  const endState = filterValueSlice.reducer(
    initialState,
    changeFavorite({ name: FilterName.favorite, isActive: true }),
  );
